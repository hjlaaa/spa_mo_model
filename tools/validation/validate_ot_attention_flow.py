...
def validate_model_flow() -> dict:
    ...
    if contexts is None:
        raise AssertionError("Full eval snapshot did not produce OT spatial contexts.")
    for section, n in sizes.items():
        ...
        if not torch.allclose(embeddings[section], snapshot["ot_embeddings"][section]):
            raise AssertionError("Dynamic semantic source is not the OT stage.")
