...
from training.config import resolve_model_config
...
def check_two_section_case(case_name, feature_dict, spatial_loc_dict, modality_order):
    ...
    model.initialize_candidate_sparse_ot_prior(
        ...
        candidate_k=200,
        attention_topk=int(config["uot"]["topk"]),
        candidate_backend="blockwise",
        epsilon=float(config["uot"]["epsilon_init"]),
        tau_a=float(config["uot"]["tau_a"]),
        tau_b=float(config["uot"]["tau_b"]),
        max_iter=int(config["uot"]["max_iter"]),
        stabilizer=float(config["ot_attention"]["delta"]),
    )
    outputs = model(
...
def expect_value_error(case_name, feature_dict, spatial_loc_dict, expected_text):
    try:
        ...
        model.initialize_candidate_sparse_ot_prior(
            ...
            initial_modality_candidate_k=100,
            candidate_k=200,
            attention_topk=int(model.config["uot"]["topk"]),
            candidate_backend="blockwise",
            epsilon=float(model.config["uot"]["epsilon_init"]),
            tau_a=float(model.config["uot"]["tau_a"]),
            tau_b=float(model.config["uot"]["tau_b"]),
            max_iter=int(model.config["uot"]["max_iter"]),
