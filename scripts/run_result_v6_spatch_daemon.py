#!/usr/bin/env python3
"""Detached SPATCH v6 dual-GraphSAGE training and result_v5-compatible analysis."""
...
def task_commands(python: str) -> list[tuple[str, list[str]]]:
    ...
    return [
        ...
        (
            "analyze_spatch",
            [
                python, str(ROOT / "scripts/analyze_spatch_result_v6_standardized.py"),
...
def validate_training() -> None:
    ...
    if summary.get("dynamic_candidate_source") != EXPECTED_DYNAMIC_SOURCE:
        raise ValueError(
            f"Unexpected dynamic OT source: {summary.get('dynamic_candidate_source')!r}"
        )
    if summary.get("architecture") != EXPECTED_ARCHITECTURE:
        raise ValueError(f"Unexpected SPATCH architecture: {summary.get('architecture')!r}")
    if summary.get("pre_post_graphsage_parameter_sharing") is not False:
        raise ValueError("Pre/post OT GraphSAGE parameters must not be shared.")
    if summary.get("ot_refresh_embedding_key") != "ot_embeddings":
        raise ValueError("Dynamic OT refresh must read ot_embeddings.")
...
def launch_daemon(python: str, from_task: str) -> None:
    ...
    if PID_PATH.is_file():
        try:
            ...
            if "run_result_v6_spatch_daemon.py" in cmdline:
                raise RuntimeError(
...
def run_suite(python: str, from_task: str) -> None:
    ...
    new_status = {
        "suite": "result_v6_spatch_dual_graphsage_delayed_ot_refresh",
        "supervisor_pid": os.getpid(),
        "started_at": now(),
        "dataset": "spatch",
        "baseline": str(RESULT_ROOT.parent / "result_v5/spatch" / VARIANT),
        "epochs": 200,
        "seed": 42,
        "device": "cuda",
    ...
    print(f"[{now()}] RESULT_V6_SPATCH_SUITE: PASS", flush=True)
