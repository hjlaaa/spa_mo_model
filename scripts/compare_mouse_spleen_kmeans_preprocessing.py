#!/usr/bin/env python3
"""Create independent raw-vs-standardized KMeans results for Mouse_Spleen.

Legacy training and analysis outputs are read-only inputs.  Each method is
aligned against its own Mouse_Spleen data copy and written to a new comparison
root. Metric CSV files retain K=2..12, while clustering directories, labels,
and spatial figures are retained only for K=5/8/10/12.
"""
...
from compare_human_lymph_node_kmeans_preprocessing import (
    f4,
    internal,
    md_table,
    metric_space,
...
SECTION_COUNTS = {"Mouse_Spleen1": 2568, "Mouse_Spleen2": 2768}
...
REPORT_PATH = Path(
    ...
    "mouse_spleen_preprocessing_comparison_report.md"
)
...
def load_spa() -> MethodData:
    ...
    return MethodData(
        ...
        Path(
            ...
            "mouse_spleen_preprocessing_comparison"
        ),
...
def load_mofa() -> MethodData:
    old = Path(
        ...
        "mouse_spleen_mofa_hvg2000_k10_iter1000"
    )
    ...
    data_dir = Path("/home/hujinlan/mofa+/data/Mouse_Spleen")
    return MethodData(
        "MOFA+",
...
def load_cosie() -> MethodData:
    run = Path(
        "/home/hujinlan/cosie_runs/mouse_spleen_cosie_rna_adt_full"
    )
    old = run / "analysis"
    ...
    data_dir = Path("/home/hujinlan/cosie/data/Mouse_Spleen")
    return MethodData(
        "COSIE",
        table_embedding,
        sections,
...
def load_spamosaic() -> MethodData:
    h5ad_path = Path(
        "/home/hujinlan/SpaMosaic-dev/runs/mouse_spleen_spamosaic/"
        "mouse_spleen_spamosaic_embeddings.h5ad"
    )
    ...
    data_dir = Path(
        "/home/hujinlan/SpaMosaic-dev/demo/data/Mouse_Spleen"
    )
    old = Path(
        "/home/hujinlan/SpaMosaic-dev/analysis/mouse_spleen_spamosaic"
    )
    return MethodData(
        "SpaMosaic",
        embedding,
...
def run_scheme(data: MethodData, scheme: str) -> None:
    ...
    config = {
        ...
        "joint_scaler_scope": (
            "none" if scheme == "raw_embedding" else "all_5336_spots"
        ),
        ...
        batch_sampling_rows.append(
            [
                ...
                int(
                    batch.get(
                        "bASW_sample_size",
                        batch.get("n_used", batch.get("n_obs", N_OBS)),
                    )
                ),
            ]
        )
...
def main() -> None:
    ...
    actions.add_argument(
        ...
        action="store_true",
        help=(
            "Redraw only K=5/8/10/12 spatial figures with point size 10 "
            "from saved labels; do not rerun KMeans or metrics."
        ),
    )
    actions.add_argument(
