...
from model.configure import get_default_preprocess_config, reject_unsupported_preprocess_config
...
def restore_section_keys(mapping, section_ids):
    """Restore positional COSIE keys to original IDs in input section order."""
    if len(set(section_ids)) != len(section_ids):
        raise ValueError("Section IDs must be unique to restore preprocessing identity.")
    internal_keys = {f"s{i + 1}" for i in range(len(section_ids))}
    if set(mapping) != internal_keys:
        raise ValueError("Preprocessing section keys do not match the input sections.")
    return {section: mapping[f"s{i + 1}"] for i, section in enumerate(section_ids)}
...
def preprocess_multisection_cosie_style(
    ...
    if config:
        reject_unsupported_preprocess_config(config)
    if config and (
        config.get("metacell", False)
        or config.get("preprocessing", {}).get("metacell", False)
    ):
