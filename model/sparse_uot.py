"""Bidirectional candidate-restricted sparse UOT priors."""
...
def update_bidirectional_candidate_sparse_uot_prior_from_embeddings(
    ...
    """Build dynamic bidirectional priors from OT-attention embeddings and contexts."""
    ...
    for left_section, right_section in zip(resolved_sections[:-1], resolved_sections[1:]):
        ...
        metadata = {
            ...
            "semantic_source": "ot",
            "context_source": (
                "ot_spatial_context" if context_weight > 0.0 else None
            ),
            "modalities_used": [modality_label],
            "modality_order": [modality_label],
            "cost_definition": (
                f"{1.0 - context_weight:.6g} * (1 - cosine(ot_left, "
                f"ot_right)) + {context_weight:.6g} * "
                "(1 - cosine(local_context_left, local_context_right))"
                if context_weight > 0.0
                else "1 - cosine(ot_left, ot_right)"
            ),
            ...
            "candidate_search_metadata": {
                "ot:left_query_right": forward["metadata"],
                "ot:right_query_left": reverse["metadata"],
            },
            "candidate_search_time_sec": float(
